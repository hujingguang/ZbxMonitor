...
TENCENT_ID=""
TENCENT_KEY=""
METRIC_LIST=["InBandwidth","InPkg","OutBandwidth","OutPkg","PkgDrop"]
...
class MonitorAPI(BaseAPI):
    ...
    def _get_monitor_data(self,tunnel_id,startTime,endTime,dataType,period=300):
	for region in self._regions:
	    self.init_client(region)
	if self._client:
	    client=self._client
	    req=models.GetMonitorDataRequest()
	    params = '{"Namespace":" QCE/DCX","MetricName":"'+dataType+'","Period":'+str(period)+',"StartTime":"'+startTime+'","EndTime":"'+endTime+'","Instances":[{"Dimensions":[{"Name":"directConnectConnId","Value":"'+tunnel_id+'"}]}]}'
		...
		if data:
		    ...
		    print "Zabbix_Tencent_Cloud_Tunnel_Info","tencent.dc.info["+sys.argv[1]+","+sys.argv[2]+"]",timestamp,data[0]*1000*1000
                else:
		    print "Zabbix_Tencent_Cloud_Tunnel_Info","tencent.dc.info["+sys.argv[1]+","+sys.argv[2]+"]",str(time.time()).split('.')[0],0
            return 

    def get_monitor_data(self,tunnel_id,dataType):
	global METRIC_LIST
        now=datetime.now()
	...
	if dataType not in METRIC_LIST:
	    ...
	    self._get_monitor_data(tunnel_id,begin_time,end_time,dataType)
